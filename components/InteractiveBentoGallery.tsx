  ...
  return (
    <>
      ...
      >
        ...
        <motion.button
          className="absolute top-2 sm:top-2.5 md:top-3 right-2 sm:right-2.5 md:right-3 flex items-center justify-center h-8 w-8 rounded-full bg-gray-200/80 text-gray-700 hover:bg-gray-300/80 text-lg leading-none backdrop-blur-sm"
          onClick={onClose}
...
const InteractiveBentoGallery: React.FC<InteractiveBentoGalleryProps> = ({ mediaItems, title, description }) => {
  ...
  return (
    <div className="mx-auto py-8" style={{ maxWidth: "var(--content-max)", paddingInline: "var(--gutter)" }}>
      <div className="mb-8">
        <span className="section-label">{title}</span>
        <p className="mt-2 editorial" style={{ maxWidth: "48ch" }}>
          {description}
        </p>
      </div>
      <motion.div
        className="grid grid-cols-2 gap-3 auto-rows-[70px]"
        initial="hidden"
        animate="visible"
